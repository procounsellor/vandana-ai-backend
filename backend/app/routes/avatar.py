...
def avatar_chat(request: ChatRequest, db: Session = Depends(get_db)):
    ...
    video_urls = try_generate_avatar(audio_chunks[0]) if audio_chunks else {}

    return {
        "conversation_id": str(conversation.id),
        "message": MessageResponse.model_validate(message),
        "audio_chunks": [base64.b64encode(c).decode("utf-8") for c in audio_chunks],
        "video": {"mp4_url": video_urls.get("mp4_url"), "hls_url": video_urls.get("hls_url")},
    }
...
async def avatar_voice_chat(
    ...
    try:
        conversation, message = chat(
            user_message=user_text, db=db, user_id=TEMP_USER_ID,
            conversation_id=conversation_id, language_code=language_code,
        )
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))
